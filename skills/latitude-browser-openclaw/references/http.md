...
Use these templates through the OpenClaw `exec` tool. On Windows, prefer `scripts/invoke_latitude_browser_api.ps1` for stable quoting and consistent JSON output. On Unix-like hosts, use the `curl` patterns below.

## Preferred Windows helper

```powershell
pwsh -File skills/latitude-browser-openclaw/scripts/invoke_latitude_browser_api.ps1 `
  -Method GET `
  -Path /api/health
```
...
```bash
BASE_URL="${LATITUDE_BROWSER_BASE_URL:-${ANT_CHROME_BASE_URL:-http://127.0.0.1:19876}}"
API_HEADER="${LATITUDE_BROWSER_API_HEADER:-${ANT_CHROME_API_HEADER:-X-Ant-Api-Key}}"

curl_ant() {
  if [ -n "${LATITUDE_BROWSER_API_KEY:-${ANT_CHROME_API_KEY:-}}" ]; then
    curl -sS -H "${API_HEADER}: ${LATITUDE_BROWSER_API_KEY:-${ANT_CHROME_API_KEY:-}}" "$@"
  else
    curl -sS "$@"
  fi
}
...
```powershell
$baseUrl = if ($env:LATITUDE_BROWSER_BASE_URL) { $env:LATITUDE_BROWSER_BASE_URL } elseif ($env:ANT_CHROME_BASE_URL) { $env:ANT_CHROME_BASE_URL } else { "http://127.0.0.1:19876" }
$apiHeader = if ($env:LATITUDE_BROWSER_API_HEADER) { $env:LATITUDE_BROWSER_API_HEADER } elseif ($env:ANT_CHROME_API_HEADER) { $env:ANT_CHROME_API_HEADER } else { "X-Ant-Api-Key" }
```
...
```powershell
$headers = @{}
if ($env:LATITUDE_BROWSER_API_KEY) { $headers[$apiHeader] = $env:LATITUDE_BROWSER_API_KEY } elseif ($env:ANT_CHROME_API_KEY) { $headers[$apiHeader] = $env:ANT_CHROME_API_KEY }
...
Use this when the user wants to see what Latitude Browser scripts already exist before choosing one to run.
...
- For scripts already bound to a target in Latitude Browser, prefer the minimal body and only override what the user explicitly asked to change.
