# latitude-browser-openclaw 使用说明

## 1. 安装 Skill 方式

把 `skills/latitude-browser-openclaw` 整个目录复制到 OpenClaw 的 `skills` 目录。
...
如果 Latitude Browser 开了 API Key，安装时补上 `-ApiKey` 或 `--api-key`。
...
如果脚本已经在 Latitude Browser 里绑定了目标实例和默认参数，通常这两句就够了。
...
- 先在 Latitude Browser 前端里把实例配置好，再让 OpenClaw 接管
- 提问时尽量写清楚实例名或 `launchCode`，优先用精确标识
- 只有 `debugReady=true` 时才适合接管
- 如果有多个匹配实例，不要让它自动选，先让它告诉你
- `browser stop` 只是断开接管，不是关闭实例
- 如果脚本已经在 Latitude Browser 里绑定好了目标实例和默认参数，提示词尽量短，不要重复描述内部 API 流程
