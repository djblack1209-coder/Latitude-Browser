...
target_skills_dir=""
config_file=""
browser_profile_name="latitude-browser"
...
api_key="${LATITUDE_BROWSER_API_KEY:-${ANT_CHROME_API_KEY:-}}"
color="#0F766E"
...
print_help() {
  cat <<'EOF'
Usage: install_latitude_browser_openclaw.sh [options]

Options:
  --target-skills-dir PATH   OpenClaw skills directory
  --config-file PATH         OpenClaw config path (for example openclaw.json)
  --browser-profile NAME     Browser profile name to create/update (default: latitude-browser)
  --base-url URL             Latitude Browser LaunchServer base URL
...
skill_name="latitude-browser-openclaw"
...
env_map["LATITUDE_BROWSER_BASE_URL"] = base_url
env_map["LATITUDE_BROWSER_API_HEADER"] = api_header
if api_key.strip():
