<#
.SYNOPSIS
Install the latitude-browser-openclaw skill into an OpenClaw skills directory and optionally merge openclaw.json.

.EXAMPLE
pwsh -File install_latitude_browser_openclaw.ps1 -TargetSkillsDir "C:\OpenClaw\skills" -ConfigFile "C:\OpenClaw\openclaw.json" -SetDefaultProfile

.EXAMPLE
pwsh -File install_latitude_browser_openclaw.ps1 -TargetSkillsDir "C:\OpenClaw\skills" -DryRun
#>

param(
    [string]$TargetSkillsDir = "",
    [string]$ConfigFile = "",
    [string]$BrowserProfileName = "latitude-browser",
    [string]$BaseUrl = "",
    [string]$ApiHeader = "",
    [string]$ApiKey = "",
    [string]$Color = "#0F766E",
...
function Show-Usage {
    Write-Host "Usage:"
    Write-Host "  pwsh -File install_latitude_browser_openclaw.ps1 -TargetSkillsDir <skills-dir> [-ConfigFile <openclaw.json>] [-SetDefaultProfile]"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -TargetSkillsDir     OpenClaw skills directory"
    Write-Host "  -ConfigFile          OpenClaw config path (for example openclaw.json)"
    Write-Host "  -BrowserProfileName  Browser profile name to create or update (default: latitude-browser)"
    Write-Host "  -BaseUrl             Latitude Browser LaunchServer base URL"
...
$skillName = "latitude-browser-openclaw"
$effectiveBaseUrl = Resolve-DefaultValue $BaseUrl (Resolve-DefaultValue $env:LATITUDE_BROWSER_BASE_URL (Resolve-DefaultValue $env:ANT_CHROME_BASE_URL "http://127.0.0.1:19876"))
$effectiveApiHeader = Resolve-DefaultValue $ApiHeader (Resolve-DefaultValue $env:LATITUDE_BROWSER_API_HEADER (Resolve-DefaultValue $env:ANT_CHROME_API_HEADER "X-Ant-Api-Key"))
$effectiveApiKey = Resolve-DefaultValue $ApiKey (Resolve-DefaultValue $env:LATITUDE_BROWSER_API_KEY (Resolve-DefaultValue $env:ANT_CHROME_API_KEY ""))
...
if (-not [string]::IsNullOrWhiteSpace($resolvedConfigFile)) {
    ...
    $envMap["LATITUDE_BROWSER_BASE_URL"] = $effectiveBaseUrl
    $envMap["LATITUDE_BROWSER_API_HEADER"] = $effectiveApiHeader
