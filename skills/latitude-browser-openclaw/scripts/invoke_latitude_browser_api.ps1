...
$effectiveApiKey = Resolve-DefaultValue $ApiKey (Resolve-DefaultValue $env:LATITUDE_BROWSER_API_KEY $env:ANT_CHROME_API_KEY)
if ([string]::IsNullOrWhiteSpace($ApiHeader) -and -not [string]::IsNullOrWhiteSpace($env:LATITUDE_BROWSER_API_HEADER)) {
    $effectiveApiHeader = $env:LATITUDE_BROWSER_API_HEADER.Trim()
} elseif ([string]::IsNullOrWhiteSpace($ApiHeader) -and -not [string]::IsNullOrWhiteSpace($env:ANT_CHROME_API_HEADER)) {
    $effectiveApiHeader = $env:ANT_CHROME_API_HEADER.Trim()
}
if ([string]::IsNullOrWhiteSpace($BaseUrl) -and -not [string]::IsNullOrWhiteSpace($env:LATITUDE_BROWSER_BASE_URL)) {
    $effectiveBaseUrl = $env:LATITUDE_BROWSER_BASE_URL.Trim()
} elseif ([string]::IsNullOrWhiteSpace($BaseUrl) -and -not [string]::IsNullOrWhiteSpace($env:ANT_CHROME_BASE_URL)) {
