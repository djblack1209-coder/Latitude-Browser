...
function Invoke-GitCommit {
    ...
    $messagePath = Join-Path $env:TEMP ("latitude-browser-commit-message-" + [guid]::NewGuid().ToString("N") + ".txt")
    try {
...
function Invoke-GitAnnotatedTag {
    ...
    $messagePath = Join-Path $env:TEMP ("latitude-browser-tag-message-" + [guid]::NewGuid().ToString("N") + ".txt")
    try {
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($messagePath, $Message, $utf8NoBom)
...
$tempRoot = Join-Path $env:TEMP "latitude-browser-public-$Version-$timestamp"
