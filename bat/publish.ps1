...
function Build-WindowsBinary {
    ...
    $binaryPath = Join-Path $repoRoot "build/bin/latitude-browser.exe"
    ...
    if (-not (Test-Path -LiteralPath $binaryPath -PathType Leaf)) {
        throw "构建产物不存在: build\bin\latitude-browser.exe"
    }
    Write-Host "✓ Latitude Browser 构建成功（正式文件名: build\bin\latitude-browser.exe）"
    Write-Host ""
}
...
function New-WindowsStaging {
    ...
    Write-Host "✓ 复制 Latitude Browser 可执行文件（正式文件名: latitude-browser.exe）"
...
function Publish-Linux {
    ...
    $archOutFile = Join-Path $env:TEMP ("latitude-browser-linux-arch-" + [guid]::NewGuid().ToString("N") + ".txt")
