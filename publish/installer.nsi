; Latitude Browser NSIS Installer Script
...
!define PRODUCT_NAME    "Latitude Browser"
!define PRODUCT_EXE     "latitude-browser.exe"
!define PRODUCT_ICON    "LatitudeBrowser.ico"
!define UNINSTALL_KEY   "Software\Microsoft\Windows\CurrentVersion\Uninstall\LatitudeBrowser"
!define INSTALL_DIR     "$PROGRAMFILES64\Latitude Browser"
...
!macro WriteCloseProcessScript HANDLE
  ...
  FileWrite ${HANDLE} "function Get-LatitudeBrowserProcesses {$\r$\n"
  ...
  FileWrite ${HANDLE} "  $$procs = Get-LatitudeBrowserProcesses$\r$\n"
  ...
  FileWrite ${HANDLE} "$$left = Get-LatitudeBrowserProcesses$\r$\n"
...
OutFile "..\publish\output\LatitudeBrowser-Setup-${VERSION}.exe"
...
!define MUI_DIRECTORYPAGE_TEXT_TOP "请选择 Latitude Browser 的安装目录。建议安装到非 C 盘的可写目录，例如 D:\software\Latitude Browser 或 E:\software\Latitude Browser；不要安装到 C 盘、Program Files 或其他受权限保护的目录。"
...
!define MUI_FINISHPAGE_RUN_TEXT "Launch Latitude Browser"
...
Section "Latitude Browser (required)" SecMain
  SectionIn RO
...
!endif
  ...
  WriteRegStr HKLM "${UNINSTALL_KEY}" "Publisher"       "Latitude Browser Team"
